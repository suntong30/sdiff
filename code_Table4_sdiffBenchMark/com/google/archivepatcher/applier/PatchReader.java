...
public class PatchReader {
  ...
  public PatchApplyPlan readPatchApplyPlan(InputStream in) throws IOException {
    ...
    byte[] expectedIdentifier = PatchConstants.IDENTIFIER.getBytes("US-ASCII"); // Literal: "GFbFv1_0"
    ...
    long deltaFriendlyOldFileSize = checkNonNegative( // Delta-friendly old archive size (8 bytes) (uint64)
        dataIn.readLong(), "delta-friendly old file size");

    // Read old file uncompression instructions.
    int numOldFileUncompressionInstructions = (int) checkNonNegative( // Num old archive uncompression ops (4 bytes) (uint32)
    ...
    for (int x = 0; x < numOldFileUncompressionInstructions; x++) { // 读取n条旧文件解压缩指令，Old archive uncompression op 1...n (variable length)
      // 指令组成：offset(long), length(long)
    ...
    int numDeltaFriendlyNewFileRecompressionInstructions = dataIn.readInt(); // Num new archive recompression ops (4 bytes) (uint32)
    checkNonNegative(
    ...
    List<TypedRange<JreDeflateParameters>> deltaFriendlyNewFileRecompressionPlan =
        new ArrayList<TypedRange<JreDeflateParameters>>(
            numDeltaFriendlyNewFileRecompressionInstructions);
    lastReadOffset = -1;
    for (int x = 0; x < numDeltaFriendlyNewFileRecompressionInstructions; x++) { // 读取n条新文件压缩指令，New archive recompression op 1...n (variable length)
      // 指令组成：offset(long), length(long), Compatibility window ID(unsigned byte), level(unsigned byte), strategy(unsigned byte), nowrap(unsigned byte)
      ...
      int strategy = (int) checkRange(dataIn.readUnsignedByte(), 0, 7, "recompression strategy");
    ...
    int numDeltaRecords = (int) checkRange(dataIn.readInt(), 1, 1, "num delta records"); // 只能为1，Num delta descriptor records (4 bytes) (uint32)
    ...
    */
    ...
    // 剩下 Delta 1...n (variable length) 没有读取，即差分包
