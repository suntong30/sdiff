...
public class PreDiffExecutor {
  ...
  public PreDiffPlan prepareForDiffing() throws IOException {
    ...
    PreDiffPlan preDiffPlan = generatePreDiffPlan(); // 生成压缩计划，即猜参数和文件对比
    end = System.currentTimeMillis();
    System.out.println("guess daflate-parameter time : " + (end - start));
    ...
    if (deltaFriendlyOldFile != null) {
      ...
      System.out.println("generate delta-friendly time : " + (end - start));
    }
  ...
  private  List<TypedRange<JreDeflateParameters>> generateDeltaFriendlyFiles(PreDiffPlan preDiffPlan)
      throws IOException {
    cZlibBuildFri.buildFriBlob(preDiffPlan.getOldFileUncompressionPlan(),originalOldFile,deltaFriendlyOldFile);
    return myGenerateDeltaFriendlyFile(preDiffPlan.getNewFileUncompressionPlan(),originalNewFile,deltaFriendlyNewFile);
  }
  public <T> List<TypedRange<T>> myGenerateDeltaFriendlyFile(List<TypedRange<T>> rangesToUncompress,
                                                                    File newApk,
                                                                    File newApkFri){
    int count = rangesToUncompress.size();
    List<TypedRange<T>> inverseRanges = new ArrayList<TypedRange<T>>(count);
    ...
    cZlibBuildFri.buildFriBlobGenerate(pos,length,count,newApk,newApkFri);
    for(int i = 0;i<count;i++){
      TypedRange<T> inverseRange =
              new TypedRange<T>(pos[i],length[i],rangesToUncompress.get(i).getMetadata());
  ...
  private PreDiffPlan generatePreDiffPlan() throws IOException {
    ...
    for (DivinationResult divinationResult : diviner.divineDeflateParameters(originalNewFile)) { // 对新版本的每一个文件猜参数
      ByteArrayHolder key =
          new ByteArrayHolder(divinationResult.minimalZipEntry.getFileNameBytes());
      originalNewArchiveZipEntriesByPath.put(key, divinationResult.minimalZipEntry); // 存入minimalZipEntry
      originalNewArchiveJreDeflateParametersByPath.put(key, divinationResult.divinedParameters); // 存入divinedParameters
    }
    // System.out.println(diviner.num7z);
