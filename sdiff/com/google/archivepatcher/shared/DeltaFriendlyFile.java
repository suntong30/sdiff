...
public class DeltaFriendlyFile {
  ...
  public static <T> List<TypedRange<T>> generateDeltaFriendlyFile(
      ...
      File file, // 待解压的文件
      OutputStream deltaFriendlyOut, // 生成的差分友好文件
      boolean generateInverse, // 若为true，则返回一个解压缩顺序颠倒的列表；若为false，返回null
      int copyBufferSize)
      throws IOException {
    ...
    try {
      oldFileRafis = new RandomAccessFileInputStream(file); // 待解压文件的随机输入流
      for (TypedRange<T> rangeToUncompress : rangesToUncompress) { // 开始逐个解压文件
        long gap = rangeToUncompress.getOffset() - lastReadOffset;
        if (gap > 0) { // 跳过不需要解压的文件，即直接复制该段内容到差分友好文件
          // Copy bytes up to the range start point
          oldFileRafis.setRange(lastReadOffset, gap); // 设置复制范围
          filteredOut.pipe(oldFileRafis, PartiallyUncompressingPipe.Mode.COPY);
        }

        // Now uncompress the range.
        oldFileRafis.setRange(rangeToUncompress.getOffset(), rangeToUncompress.getLength()); // 设置解压范围
        ...
        filteredOut.pipe(oldFileRafis, PartiallyUncompressingPipe.Mode.UNCOMPRESS_NOWRAP); // 这里解压

        lastReadOffset = rangeToUncompress.getOffset() + rangeToUncompress.getLength(); // 调整文件指针
      ...
      // Finish the final bytes of the file 复制剩余数据
